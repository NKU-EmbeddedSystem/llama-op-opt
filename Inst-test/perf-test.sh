...
if [ ! -d "./res/perf_CSR_spMM_small.txt" ];then
for sp in "${sparsity[@]}"
    do
    ./build/test-CSR-spMM-SMALL -s $sp >> ./res/perf_CSR_spMM_small.txt
    done
fi
...
if [ ! -d "./res/perf_CSR_spMM_medium.txt" ];then
for sp in "${sparsity[@]}"
    do
    ./build/test-CSR-spMM-MEDIUM -s $sp >> ./res/perf_CSR_spMM_medium.txt
    done
fi
...
if [ ! -d "./res/perf_CSR_spMM_large.txt" ];then
for sp in "${sparsity[@]}"
    do
    ./build/test-CSR-spMM-LARGE -s $sp >> ./res/perf_CSR_spMM_large.txt
    done
fi
...
if [ ! -d "./res/perf_CSR_spMM_super_large.txt" ];then
for sp in "${sparsity[@]}"
    do
    ./build/test-CSR-spMM-SUPER_LARGE -s $sp >> ./res/perf_CSR_spMM_super_large.txt
    done
fi